    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.75} style={{ alignItems: 'center', width: 72 }}>
      <View style={{
        ...
        {imageUri ? (
          ...
          <Text style={{ fontSize: 24, fontWeight: '900', color: ORANGE, opacity: 0.8 }}>{initial}</Text>
        )}
      </View>
...
function DirectVideoPlayer({ source, onEnd }: { source: string; onEnd: () => void }) {
  ...
  return (
    <VideoView player={player} style={introStyles.video} contentFit="contain" nativeControls fullscreenOptions={{ enable: true }} />
  );
}
...
function YoutubeExternalPlayer({ source, title, onClose }: { source: string; title: string; onClose: () => void }) {
  return (
    <View style={introStyles.youtubeFallback}>
      ...
      <Heading size="xl" className="text-white text-center" numberOfLines={2}>{title}</Heading>
      ...
      <TouchableOpacity activeOpacity={0.85} style={introStyles.youtubeButton} onPress={() => void Linking.openURL(source)}>
        ...
        {youtubeEmbedUrl ? (
          <View style={introStyles.caption}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Heading size="xl" className="text-white mt-1" numberOfLines={2}>{title}</Heading>
            </View>
          </View>
        ) : null}
      </View>
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={videoCardStyles.card}>
      <View style={videoCardStyles.thumb}>
        {imageUri ? <Image source={{ uri: imageUri }} style={videoCardStyles.thumbImage} contentFit="cover" /> : null}
      ...
      <View style={{ flex: 1 }}>
        <Text size="2xs" muted bold className="uppercase tracking-widest">Video bối cảnh</Text>
        <Text style={videoCardStyles.title} numberOfLines={2}>Xem lại: {title}</Text>
      </View>
    </TouchableOpacity>
  );
}
...
export default function ContextDetailScreen() {
  ...
  const closeVideo = () => { setSkippedIntroId(resolvedId ?? null); setManualVideoId(null); };
        ...
        <VStack style={{ paddingHorizontal: 24, paddingTop: 28, alignItems: 'center' }}>
          ...
          {(yearText || ctx.period || ctx.location) ? (
            <VStack style={{ alignItems: 'center', marginBottom: 28, gap: 4 }}>
              {yearText ? (
                <Text style={{ color: ORANGE, fontWeight: '600', fontSize: 15, textAlign: 'center' }}>
                  {yearText}
                </Text>
              ) : null}
              {ctx.period ? (
          ...
          {/* Quiz banner */}
          <TouchableOpacity onPress={() => router.push('/quiz')} activeOpacity={0.85} style={quizStyles.banner}>
...
const introStyles = StyleSheet.create({
  ...
  topBar:       { position: 'absolute', top: 0, left: 0, right: 0, alignItems: 'flex-end', paddingHorizontal: 16, paddingTop: 52 },
  closeButton:  { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 999, backgroundColor: BrandColors.overlayStrong, borderWidth: 1, borderColor: BrandColors.borderSubtle, paddingHorizontal: 18, paddingVertical: 10 },
  closeText:    { color: Colors.dark.text, fontSize: 13, fontWeight: '800' },
  caption:      { position: 'absolute', left: 20, right: 20, bottom: 36, flexDirection: 'row', alignItems: 'center', gap: 12 },
  playBadge:    { width: 42, height: 42, borderRadius: 21, backgroundColor: BrandColors.primary, alignItems: 'center', justifyContent: 'center' },
  youtubeFallback: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 16, paddingHorizontal: 28 },
  playBadgeLarge:  { width: 74, height: 74, borderRadius: 37, backgroundColor: BrandColors.primary, alignItems: 'center', justifyContent: 'center', marginBottom: 6 },
  youtubeButton:   { width: '100%', maxWidth: 320, height: 52, borderRadius: 16, backgroundColor: BrandColors.primary, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
  youtubeButtonText: { color: Colors.dark.text, fontSize: 15, fontWeight: '800' },
  continueButton:  { paddingHorizontal: 16, paddingVertical: 10 },
  continueText:    { color: BrandColors.primarySoft, fontSize: 13, fontWeight: '700' },
});

const videoCardStyles = StyleSheet.create({
  card: {
    flexDirection: 'row', alignItems: 'center', gap: 14,
    marginBottom: 28, borderRadius: 18, borderWidth: 1,
    borderColor: BrandColors.primaryBorder,
    backgroundColor: BrandColors.primaryMuted,
    padding: 12, width: '100%',
  },
  thumb:        { width: 92, height: 62, borderRadius: 14, overflow: 'hidden', backgroundColor: Colors.dark.backgroundElement, alignItems: 'center', justifyContent: 'center' },
  thumbImage:   { width: '100%', height: '100%', position: 'absolute' },
  thumbOverlay: { ...StyleSheet.absoluteFill, backgroundColor: BrandColors.overlay },
  playButton:   { width: 34, height: 34, borderRadius: 17, backgroundColor: BrandColors.primary, alignItems: 'center', justifyContent: 'center' },
  title:        { color: Colors.dark.text, fontSize: 14, fontWeight: '800', lineHeight: 20, marginTop: 4 },
});
